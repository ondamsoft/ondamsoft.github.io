...
function toggleCrypto() {
  ...
  var title = document.querySelector('.crypto-title');
  if (list) list.classList.toggle('open');
  if (note) note.classList.toggle('open');
  if (title) {
    var isOpen = list && list.classList.contains('open');
    var text = title.getAttribute('data-i18n') ? (LANG[document.documentElement.lang] || {})['donate.crypto_title'] || title.textContent : title.textContent;
    title.textContent = text.replace(/ [▼▲]$/, '') + (isOpen ? ' ▲' : ' ▼');
  }
}
